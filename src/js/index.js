...
export async function upload(data) {
  ...
  if (docSnap.exists()) {
    ...
    // update the fields except for the timestamp
    console.log("updating doc!");
    await updateDoc(docRef, {
      badPosture: increment(data.badPosture),
      goodPosture: increment(data.goodPosture),
      breakTime: increment(data.breakTime),
      cameraTime: increment(data.cameraTime),
      useTime: increment(data.useTime),
      countDistance: increment(data.countDistance),
      countHeadTowardShoulder: increment(data.countHeadTowardShoulder),
      countHeadTurned: increment(data.countHeadTurned),
      countHeadUporDown: increment(data.countHeadUporDown),
    });
  } else {
...
function RandomizeTips() {
  let DailyPostureTipIndex = Math.floor(Math.random() * (7 - 0) + 0);
